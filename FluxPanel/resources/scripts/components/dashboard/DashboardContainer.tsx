        ...
        <PageContentBlock title={'Servers'} showFlashKey={'dashboard'}>
            <div css={tw`mx-auto max-w-[1180px]`}>
                ...
                <section css={tw`grid min-h-[90px] grid-cols-1 border-b border-t border-neutral-700 sm:grid-cols-3`}>
                    ...
                    <div css={tw`px-[22px] py-[18px]`}><p css={tw`text-xl font-semibold text-neutral-100`}>—</p><p css={tw`mt-1 text-[9px] font-medium uppercase tracking-wider text-neutral-400`}>Server uptime</p><p css={tw`mt-1 text-[9px] text-neutral-600`}>Open a server to view its uptime</p></div>
                </section>
                <div css={tw`flex h-[58px] items-center`}><h2 css={tw`text-sm font-semibold text-neutral-100`}>Your servers</h2></div>
                {!servers ? <Spinner centered size={'large'} /> : <Pagination data={servers} onPageSelect={setPage}>{({ items }) => items.length > 0 ? <div css={tw`overflow-x-auto`}><div css={tw`grid min-w-[900px] border-b border-neutral-700 pb-2 text-[9px] font-medium uppercase tracking-wider text-neutral-400`} style={{ gridTemplateColumns: 'minmax(250px, 2.4fr) minmax(120px, 1.2fr) minmax(110px, 1fr) repeat(3, minmax(90px, 0.8fr))' }}><span>Server</span><span>Region</span><span>Status</span><span>CPU</span><span>Memory</span><span>Disk</span></div>{items.map((server) => <ServerRow key={server.uuid} server={server} />)}</div> : <p css={tw`py-8 text-center text-sm text-neutral-500`}>{showOnlyAdmin ? 'There are no other servers to display.' : 'There are no servers associated with your account.'}</p>}</Pagination>}
            </div>
        </PageContentBlock>
    );
};
