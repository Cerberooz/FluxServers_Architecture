...
type Summary = { implementation?: string; minecraft_version?: string; memory_mb?: number; cpu_percent?: number; spark?: { available: boolean; built_in: boolean }; message?: string; report_id?: string; tps?: number; mspt_p95?: number; network?: Network; server_health?: { score?: number; status?: string }; plugin_usage?: { source: string; percent: number }[] };
...
export default () => {
    ...
    const health = selected?.summary?.server_health;
    ...
    return <ServerContentBlock title={'Optimizer'} showFlashKey={'server:optimizer'}>
        {!response ? <Spinner size={'large'} centered /> : <>
            ...
            <div css={tw`mb-6 grid gap-3 md:grid-cols-4`}>
                ...
                <Stat label={'Server Health'} value={health?.score !== undefined ? `${health.score}/100 ${health.status || ''}` : 'No performance report'} />
            ...
            {selected && <section css={tw`space-y-4`}>
