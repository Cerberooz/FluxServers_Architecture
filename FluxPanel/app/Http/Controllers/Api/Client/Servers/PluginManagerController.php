...
class PluginManagerController extends ClientApiController
{
    ...
    public function search(Request $request, Server $server, ModrinthPluginService $service): array { $this->read($request, $server); $data = $request->validate(['query' => 'nullable|string|max:100', 'page' => 'nullable|integer|min:1']); return $service->search($server, $data['query'] ?? '', (int) ($data['page'] ?? 1)); }
